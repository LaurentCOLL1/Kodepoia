...
def _workspace(root: Path) -> tuple[str, Path]:
    ...
    manifest = engine.apply(root, engine.preview(root, definition, model, lineage))
    return manifest.digest(), root


def _request(manifest_sha: str, *, target_sdk: int = 36) -> AndroidBuildRequest:
...
def test_r13_4_dynamic_versions_and_unofficial_sources_fail_closed() -> None:
    with pytest.raises(ValueError, match="explicit numeric"):
        replace(_toolchain(), android_gradle_plugin="9.3.+")
    with pytest.raises(ValueError, match="developer.android.com"):
        replace(_toolchain(), source_urls=("https://example.com/android",))


def test_r13_4_unsafe_environment_is_rejected_not_forwarded() -> None:
...
def test_r13_4_source_manifest_must_match_every_scaffold_file(tmp_path: Path) -> None:
    ...
    (source / "app/build.gradle.kts").write_text("tampered\n", encoding="utf-8")
...
def test_r13_4_build_overlay_is_deterministic_and_isolated(tmp_path: Path) -> None:
    manifest_sha, source = _workspace(tmp_path / "source")
    overlay_a = prepare_build_staging(source, tmp_path / "build-a", _toolchain())
    overlay_b = prepare_build_staging(source, tmp_path / "build-b", _toolchain())
    ...
    catalog = (tmp_path / "build-a/gradle/libs.versions.toml").read_text(encoding="utf-8")
    root_build = (tmp_path / "build-a/build.gradle.kts").read_text(encoding="utf-8")
...
def test_r13_4_valid_apk_and_aab_structural_evidence(tmp_path: Path) -> None:
    ...
    _zip(apk, {
        "AndroidManifest.xml": b"manifest", "classes.dex": b"dex", "resources.arsc": b"resources",
        "lib/arm64-v8a/libfixture.so": b"native",
    })
    _zip(aab, {
        "base/manifest/AndroidManifest.xml": b"manifest", "base/dex/classes.dex": b"dex",
        "base/resources.pb": b"resources", "base/lib/arm64-v8a/libfixture.so": b"native",
    })
...
def test_r13_4_fake_or_traversing_packages_fail_closed(tmp_path: Path) -> None:
    ...
    _zip(escaping, {
        "../owned": b"bad", "base/manifest/AndroidManifest.xml": b"manifest",
        "base/dex/classes.dex": b"dex", "base/resources.pb": b"resources",
    })
...
def test_r13_4_pass_requires_both_artifacts_and_api_36() -> None:
    with pytest.raises(ValueError, match="APK and AAB"):
        AndroidBuildEvidence(
            ...
            source_workspace_manifest_sha256="b" * 64, overlay_manifest_sha256="c" * 64,
            toolchain=_toolchain(), request=_request("b" * 64), status=AndroidBuildStatus.PASS,
            duration_seconds=1.0, artifacts=(),
        )

    with pytest.raises(ValueError, match="target SDK 36"):
        AndroidBuildEvidence(
            schema_version=1, source_sha="a" * 40, runner_os="Linux",
            source_workspace_manifest_sha256="b" * 64, overlay_manifest_sha256="c" * 64,
