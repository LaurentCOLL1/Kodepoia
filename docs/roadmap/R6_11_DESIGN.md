...
**Manual intervention:** CONDITIONAL — not triggered by the foundation design

## Objective

Create a local-first, auditable dependency/asset BOM and license-evidence layer that preserves exact provenance, unresolved versions, unknown licenses and policy uncertainty instead of manufacturing release/legal conclusions.
...
SPDX distinguishes declared license information found in an artifact from a concluded license, and distinguishes an explicit `NoAssertionLicense` known-unknown from a missing relationship. R6.11 mirrors those semantics with optional `declared_license` and required `concluded_license` evidence.

CycloneDX 1.7 is current stable interoperability context. R6.11 does not silently replace SPDX with CycloneDX and does not implement CycloneDX as a completion requirement.
...
Kodepoia's current proprietary repository license may be represented as `LicenseRef-Kodepoia-Proprietary` when its `LICENSE` file SHA-256 is supplied as custom-license evidence. This does not pretend the proprietary terms are an SPDX-listed license.

Expression handling is deliberately limited to stable lexical normalization/guarding needed by R6.11. It does not claim to replace the official SPDX License List parser or perform legal interpretation.
...
- project/scope;
- explicit inventory completeness and review provenance;
- SPDX baseline/serialization version;
- deterministic counts/blockers/status;
- sorted components;
...
- WARN: incomplete inventory, unresolved component or unknown integrity;
- PASS: complete inventory and all included components resolved with non-unknown integrity.

### `LicenseReport`

Derived from a validated BOM + license policy and bound to the BOM evidence SHA-256. Status:

- UNKNOWN: no decisions;
- FAIL: any DENY;
- WARN: incomplete inventory or any WARN/UNKNOWN;
- PASS: complete inventory and all decisions ALLOW.

Counts, blockers, score and policy fingerprint are derived/tamper checked.

## SPDX compatibility view

`KodeBOM.spdx_compatibility_view()` exposes versioned package/license/integrity normalization using the current SPDX 3.0.1 context. It explicitly includes `conformance_claim: false`.

R6.11 does **not** claim that this compact view is a fully conformant SPDX JSON-LD document. Official SPDX conformance also requires structural and semantic validation against the official schema/ontology; that is outside the minimum R6.11 foundation unless added later through a governed validator.

## R6 integrations

- Health `dependencies` from BOM resolution/integrity evidence;
- Health `licenses` from license-policy evidence;
- stable R6.3 test IDs:
  - `bom:<component-id>`;
  - `license:<component-id>`;
- unknown/unresolved/warn evidence maps to SKIP rather than fake PASS;
...
**CONDITIONAL, not currently triggered.**

Foundation acceptance can prove both known licenses and unresolved `NOASSERTION` behavior with fixtures and the real Kodepoia manifest. User intervention is required only if an acceptance-critical real component must receive a specific license conclusion and trusted authoritative evidence remains genuinely ambiguous. In that case, ambiguity remains UNKNOWN/blocking until governed resolution or removal; no SPDX ID is invented.
