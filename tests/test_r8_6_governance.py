...
def _ingest(tmp_path: Path, *, name: str = "asset.txt", content: str = "asset"):
    ...
    store.add_project_reference(ProjectAssetReference("game", revision.revision_id, f"assets/{name}"))
...
def test_unknown_license_is_blocked_before_any_export_write(tmp_path: Path) -> None:
    ...
    assert not any(export_root.glob(".kodepoia-export-stage-*")) if export_root.exists() else True
...
def test_derived_asset_retains_source_revision_requirement(tmp_path: Path) -> None:
    ...
    from kodepoia.assets.transforms import DeterministicTextTransform, TransformRecipe, TransformRegistry, TransformService

    registry = TransformRegistry()
    registry.register(DeterministicTextTransform())
    transform_service = TransformService(store, registry)
    derived = transform_service.execute(
        TransformRecipe("text-normalize", 1, {"uppercase": True}),
        (source.revision_id,),
        output_asset_id=AssetId.from_seed("test", "derived.txt"),
        output_kind=AssetKind.DOCUMENT,
        reuse_scope=ReuseScope.EXPORTABLE,
    )
    service = AssetGovernanceService(store, _policy())
    evidence = AssetLicenseEvidence(derived.revision_id, assertions=(_assertion("MIT"),))
    component = service.bom_component(derived.revision_id, evidence)
