...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build the canonical anti-circular R15 integrated acceptance report."
    )
    parser.add_argument(
        "--source-sha",
        required=True,
        help="Immutable accepted R15.17 technical source SHA",
    )
    parser.add_argument("--repository-root", default=".")
    parser.add_argument("--output", default="docs/roadmap/R15_INTEGRATED_ACCEPTANCE.json")
    args = parser.parse_args()

    root = Path(args.repository_root).resolve()

    def read_bytes(source: str) -> bytes:
        return _read_repository_bytes(root, source)
