...
`DatasetPolicy` binds the explicit seed, sanitizer digest, R15.3 governance-policy digest, R15.4 dedup-policy digest, split weights, optional task/domain filters, duplicate handling and deterministic per-domain group cap into one canonical SHA-256 policy digest. Any mismatched sanitizer, governance lineage, dedup authority or contamination report fails closed rather than silently rebuilding under changed assumptions.
...
The default duplicate policy exports only the representative selected by R15.4. A governed `keep_group` policy may preserve all eligible variants, but group atomicity still prevents cross-split leakage. Optional domain balancing ranks complete groups deterministically and never samples individual rows across group boundaries.
...
## Immutable artifacts and non-leakage
...
The canonical dataset manifest intentionally contains no raw training payload and no governed `storage_key`. It records source/content/contract/row/representation digests, R15.4 group/split identity, license expression, transformation lineage, complete policy descriptor, selection exclusions, per-split domain/task/language/group statistics and export digests.
