...
from .rig_bootstrap import RIG_BOOTSTRAP_SOURCE
...
_POLICY_VERSION = "r10.6-v1"
_OUTPUT_NAME = "rig_output.blend"


class RigRunner:
    """Governed R10.6 rig/skinning executor layered on the accepted Blender process boundary."""
    ...
    def _confined_blend(self, path: Path) -> Path:
        ...
        if not _is_within(candidate, self.input_root) or not candidate.is_file() or candidate.suffix.lower() != ".blend":
            raise BlenderBoundaryError("Rig input must be a .blend file inside its governed root")
        return candidate

    def _prepare(self, profile: RigProfile, *, source_sha: str, input_blend: Path) -> Path:
        ...
        if any(workspace.iterdir()):
            raise BlenderBoundaryError("R10.6 rig staging workspace must be empty")
        source = self._confined_blend(input_blend)
        if _sha256_file(source) != profile.input_blend_sha256:
            raise BlenderBoundaryError("Rig input digest does not match profile lineage")
        shutil.copyfile(source, workspace / "input.blend")
        job = {"schema": "kodepoia.blender.rig_job", "version": 1, "source_sha": source_sha, "policy_version": _POLICY_VERSION, "profile_digest": profile.digest, "input_blend_sha256": profile.input_blend_sha256, "profile": profile.to_dict(), "normalized_explicit_weights": profile.normalized_explicit_weights()}
        _atomic_write(workspace / "rig_job.json", canonical_json_bytes(job))
        _atomic_write(workspace / "rig_bootstrap.py", RIG_BOOTSTRAP_SOURCE.encode("utf-8"))
        return workspace

    def _load_result(self, workspace: Path) -> dict[str, Any]:
        path = (workspace / "rig_result.json").resolve(strict=False)
        if not _is_within(path, workspace) or not path.is_file():
            raise BlenderProtocolError("Rig result is missing or escaped staging")
        if path.stat().st_size > self.blender_runner.limits.max_result_bytes:
            raise BlenderProtocolError("Rig result exceeds size limit")
        try:
            payload = json.loads(path.read_text(encoding="utf-8"))
        except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
            raise BlenderProtocolError("Rig result is not valid UTF-8 JSON") from exc
        if not isinstance(payload, dict) or payload.get("schema") != "kodepoia.blender.rig_measurements" or payload.get("version") != 1:
            raise BlenderProtocolError("Unexpected rig result schema/version")
        if payload.get("status") not in {"pass", "fail"} or not isinstance(payload.get("blockers"), list):
            raise BlenderProtocolError("Malformed rig result status/blockers")
        if not isinstance(payload.get("armature"), dict) or not isinstance(payload.get("meshes"), dict):
            raise BlenderProtocolError("Rig result armature/meshes must be objects")
        return payload
    ...
    def _verify_output(self, workspace: Path, record: Any) -> dict[str, Any]:
        if not isinstance(record, dict) or record.get("filename") != _OUTPUT_NAME:
            raise BlenderProtocolError("Rig output must be rig_output.blend")
        path = (workspace / _OUTPUT_NAME).resolve(strict=True)
        if not _is_within(path, workspace) or not path.is_file():
            raise BlenderProtocolError("Rig output escapes staging")
        size = path.stat().st_size
        digest = _sha256_file(path)
        if record.get("bytes") != size or record.get("sha256") != digest:
            raise BlenderProtocolError("Rig output identity mismatch")
    ...
    def run(self, executable: Path, profile_payload: dict[str, Any], *, source_sha: str, input_blend: Path) -> dict[str, Any]:
        profile = RigProfile.from_dict(profile_payload)
        ...
        argv = self.blender_runner.boundary.build_job_argv(blender, workspace / "rig_bootstrap.py")
        ...
        if result is not None:
            ...
            if result.get("status") != "pass":
                ...
                blockers.append("rig_execution_failed")
            else:
                try:
                    report = evaluate_rig_measurements(profile, result)
        ...
        return {"schema": "kodepoia.blender.rig_manifest", "version": 1, "source_sha": source_sha, "policy_version": _POLICY_VERSION, "rig_id": profile.rig_id, "profile_digest": profile.digest, "input_blend_sha256": profile.input_blend_sha256, "status": status, "blockers": unique, "report": report, "report_digest": report.get("report_digest") if report else None, "artifact": artifact, "lineage": {"parent_sha256": profile.input_blend_sha256, "derived_sha256": artifact.get("sha256") if artifact else None}, "process": {"returncode": process.returncode, "timed_out": process.timed_out, "cancelled": process.cancelled, "stdout_truncated": process.stdout_truncated, "stderr_truncated": process.stderr_truncated}}
