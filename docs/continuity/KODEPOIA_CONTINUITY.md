...
- R15.3 : **IN_PROGRESS** — dedicated branch `r15/03-sanitization-governance` from normalized R15.2 `main` `ac1101d9e67f5e0b36d8daaca2287e625d88787b`. START scope: deterministic sanitizer/redaction + transformation digests, source/privacy/provenance/license decisions fail-closed, SPDX-compatible expression handling, safe reports and lineage-aware revocation; manual NONE. R15.4+ remain unauthorized.
...
**R15.3 START-sync is active on `r15/03-sanitization-governance`. Implement and accept only R15.3; R15.4+ remain unauthorized until R15.3 merge + unique post-merge continuity normalization.**
