...
from kodepoia.kodestudio.fr_catalogs import R13_WIZARD_FR, translated
...
class R13WizardTranslator:
    ...
    def text(self, message_id: str, **values: Any) -> str:
        if self.locale == "fr":
            try:
                return translated(R13_WIZARD_FR, message_id, **values)
            except KeyError as exc:
                raise KeyError(f"missing French R13 wizard translation: {message_id}") from exc
...
def registered_r13_wizard_messages() -> Mapping[str, str]:
    return {message.id: message.forms["other"] for message in R13_WIZARD_SOURCE_CATALOG.messages}
