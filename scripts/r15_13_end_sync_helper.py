...
TECHNICAL_SOURCE = "f0dfcd1ed3e9d2382ad44efdcd2ec05dbac1b7ac"
NORMALIZED_BASE = "ca625d51808de6c1f9c950faecc2aa785e7a757d"

plan_path = Path("docs/roadmap/R15_PLAN.md")
plan = plan_path.read_text(encoding="utf-8")
old_checkpoint = (
    "R15.1–R15.12 are COMPLETE + NORMALIZED. R15.13 is IN_PROGRESS from normalized `main` "
    f"`{NORMALIZED_BASE}`; R15.14–R15.17 remain PLANNED."
)
new_checkpoint = (
    "R15.1–R15.12 are COMPLETE + NORMALIZED. R15.13 is COMPLETE on its accepted END candidate "
    f"from normalized `main` `{NORMALIZED_BASE}`; R15.14–R15.17 remain PLANNED and unauthorized "
    "until R15.13 post-merge normalization."
)
if old_checkpoint not in plan:
    raise SystemExit("R15.13 checkpoint marker not found")
...
old_row = (
    ...
    "IN_PROGRESS | CONDITIONAL / NOT TRIGGERED | R15.10–R15.12 + R3 |"
)
new_row = old_row.replace("IN_PROGRESS", "COMPLETE")
if old_row not in plan:
    raise SystemExit("R15.13 status row not found")
plan = plan.replace(old_row, new_row, 1)
section_start = plan.index(
    "# R15.13 — Ollama import/Modelfile packaging, base-binding + local runtime verification"
)
section_end = plan.index("\n---\n\n# R15.14 —", section_start)
section = plan[section_start:section_end]
old_completion = "## Completion record\n\nTo be appended when accepted."
new_completion = f'''## Completion record

**COMPLETE — immutable technical source accepted; fresh exact-END gates required before merge.**

- clean START / normalized R15.12 `main`: `{NORMALIZED_BASE}`;
- immutable technical source: `{TECHNICAL_SOURCE}`;
...
if old_completion not in section:
    raise SystemExit("R15.13 completion placeholder not found")
section = section.replace(old_completion, new_completion, 1)
plan = plan[:section_start] + section + plan[section_end:]

# Repair only stale status cells in the phase index; historical completion records stay untouched.
lines = plan.splitlines()
normalized_ids = {f"R15.{n}" for n in range(1, 13)}
for index, line in enumerate(lines):
    if not line.startswith("| R15."):
        continue
    parts = line.split("|")
    if len(parts) < 4:
        continue
    ...
    if ident in normalized_ids and status == "COMPLETE":
...
first_line_end = continuity.index("\n")
continuity = (
    "> Kodepoia, architecture v1.0 gelée. **R1–R14 COMPLETE + NORMALIZED. R15 planning ACCEPTED + NORMALIZED. "
    "R15.1–R15.12 COMPLETE + NORMALIZED; R15.13 COMPLETE; R15.14–R15.17 PLANNED.** "
    f"R15.13 immutable technical source `{TECHNICAL_SOURCE}` passed R15.13 #3 / `33327931401` Ubuntu + Windows, "
    "R0 #2182 / `33327931448` Ubuntu + Windows, Python Core #2157 / `33327931386` and KodeStudio UI Smoke #2122 / `33327931328`. "
    "The R15.13 END-sync candidate must now receive fresh exact-head R15.13/R0/Python/UI gates before PR #321 may merge with exact expected-head protection; technical-source evidence is not reusable for that merge decision. "
    "Manual state R15.13: CONDITIONAL / NOT TRIGGERED. R15.14 remains unauthorized until the unique post-merge continuity-only R15.13 normalization passes fresh R0/Python/UI and merges.\n"
    + continuity[first_line_end + 1 :]
)
old_bullet_start = continuity.index("- R15.13 : **IN_PROGRESS**")
old_bullet_end = continuity.index("\n", old_bullet_start)
new_bullet = (
    f"- R15.13 : **COMPLETE** — clean START / normalized R15.12 `main` `{NORMALIZED_BASE}`; immutable technical source `{TECHNICAL_SOURCE}`; "
    "R15.13 #3 / `33327931401` SUCCESS Ubuntu + Windows with 19 focused/dependency tests per OS plus Ruff/compile/schema; "
    "R0 #2182 / `33327931448` SUCCESS Ubuntu + Windows; Python Core #2157 / `33327931386` SUCCESS; KodeStudio UI Smoke #2122 / `33327931328` SUCCESS. "
    "Exact FROM/ADAPTER base binding, deterministic Modelfile, namespaced candidate tags, create/show/tags/run lifecycle, digest/details capture, structured/tool probes, KodeBench veto and governed rollback are accepted in core fixtures. "
    "Public push, remote authoritative Ollama and silent active-tag replacement remain forbidden. Manual CONDITIONAL / NOT TRIGGERED; no real local-model packaging claim is made. "
    "Final documentary END-head requires fresh R15.13/R0/Python/UI before PR #321 merge; post-merge normalization remains mandatory before R15.14."
)
continuity = continuity[:old_bullet_start] + new_bullet + continuity[old_bullet_end:]

table_start = continuity.index("## R15 status index")
table_end = continuity.index("\n## Next authorized action", table_start)
new_table = '''## R15 status index
...
'''
continuity = continuity[:table_start] + new_table + continuity[table_end:]
next_start = continuity.index("## Next authorized action")
next_end = continuity.index("\n## Permanent R-phase execution rule", next_start)
...
'''
continuity = continuity[:next_start] + next_text + continuity[next_end:]
continuity_path.write_text(continuity, encoding="utf-8")

acceptance = f'''# R15.13 — Ollama import / Modelfile packaging acceptance
...
These runs qualify only immutable technical source `{TECHNICAL_SOURCE}`. They MUST NOT be reused as final merge evidence after this END-sync changes documentation bytes.
...
'''
Path("docs/roadmap/R15_13_ACCEPTANCE.md").write_text(acceptance, encoding="utf-8")
