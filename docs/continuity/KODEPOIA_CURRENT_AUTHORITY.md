...
**Current development direction:** Roadmap V2; V2.1, V2.2 and V2.3 are COMPLETE + NORMALIZED. V2.4 planning and **V2.4.1 — Accelerator topology and provider truth are COMPLETE + NORMALIZED**. V2.4.1 implementation PR `#518` was qualified **30/30** on exact final head `544b7d172499594f32c464c46753bfc5fb378fe0`, with deterministic V2.4.1 acceptance **22/22 PASS on Ubuntu and 22/22 PASS on Windows**, common evidence SHA-256 `3049f1d4fb07d8edb4d8722ffaf1a1d8e503186972eec2dfb000a5057f519c6d`, then merged with `expected_head_sha` protection as `3ebed48b8aa113635ddc9516827d867ce346fe0f`. **V2.4.2 — Strategy, effective-batch and resource planning contract is the only authorized implementation subdivision.** V2.4.3+ remain unauthorized until V2.4.2 is implemented, exact-head qualified, merged and post-merge normalized.
...
V2.3 is **COMPLETE + NORMALIZED** through V2.3.6. V2.4 planning and **V2.4.1 are COMPLETE + NORMALIZED**. Planning PR `#516` remains accepted at **25/25** on exact final head `8c78ff19cf05b3009a05e2e5337ce31303e9d8b7`, merge `a14190f529a465e8e42ee0dd90a0248bc38e1b9c`. V2.4.1 implementation PR `#518` was qualified **30/30** on exact final head `544b7d172499594f32c464c46753bfc5fb378fe0`; its deterministic acceptance passed **22/22 on Ubuntu** and **22/22 on Windows** with common evidence SHA-256 `3049f1d4fb07d8edb4d8722ffaf1a1d8e503186972eec2dfb000a5057f519c6d`, Ubuntu artifact `10614179278` and Windows artifact `10614546908`. It merged with `expected_head_sha` protection as `3ebed48b8aa113635ddc9516827d867ce346fe0f`. Accepted product truth preserves the historical R15.8 capability schema v1, adds a separate versioned topology report, enumerates actual CUDA/ROCm devices with per-device VRAM, keeps legacy scalar VRAM bound to device 0, separates Kaggle provider request from observed topology, rejects mismatches fail closed, and permits only explicit `single_gpu` selection using the selected device budget. No distributed launch or pooled-VRAM behavior was introduced. **This normalization authorizes V2.4.2 — Strategy, effective-batch and resource planning contract only.** V2.4.3+ remain unauthorized until V2.4.2 is implemented, exact-head qualified, merged and post-merge normalized. V2.5+, release/TUF/updater mutation and R20 reopening remain unauthorized.
...
5. `docs/roadmap/V2_4_KAGGLE_T4X2_PRODUCTION_MULTIGPU.md` for normalized V2.4 planning/V2.4.1 authority and the current V2.4.2 boundary;
