...
def test_read_is_allowed(tmp_path: Path) -> None:
    decision = guardian(tmp_path).authorize(ActionRequest(ActionType.READ, "test", str(tmp_path / "a.txt")))
    assert decision.kind is DecisionKind.ALLOW


def test_bulk_delete_requires_confirmation(tmp_path: Path) -> None:
    ...
    confirmed = guardian(tmp_path).authorize(request, confirmed=True)
    assert confirmed.kind is DecisionKind.ALLOW


def test_downloaded_unsandboxed_execution_is_denied(tmp_path: Path) -> None:
    request = ActionRequest(ActionType.EXECUTE, "test", downloaded=True, sandboxed=False, metadata={"executable": "python"})
