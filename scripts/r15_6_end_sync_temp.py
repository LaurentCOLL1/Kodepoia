...
DESIGN = f"""# R15.6 — KodeBench v2 design

**Status:** COMPLETE — technical design implemented  
**Clean START / branch point:** `{START_SHA}`  
...
Technical source `{TECHNICAL}` passed:
...
Path("docs/roadmap/R15_6_DESIGN.md").write_text(DESIGN, encoding="utf-8", newline="\n")
Path("docs/roadmap/R15_6_ACCEPTANCE.md").write_text(ACCEPTANCE, encoding="utf-8", newline="\n")
...
checkpoint = (
    ...
    "R15.1–R15.5 are COMPLETE + NORMALIZED. R15.6 is COMPLETE on this documented END candidate "
    f"after technical qualification of `{TECHNICAL}`; fresh exact-head R15.6/R0/Python/UI gates, "
    "expected-head merge and one continuity-only normalization remain required before R15.7. "
    "R15.7–R15.17 remain PLANNED."
)
for index, line in enumerate(lines):
    if line.startswith("**Execution checkpoint:**"):
        lines[index] = checkpoint
        break
else:
...
- clean START / normalized R15.5 branch point: `{START_SHA}`;
...
old = "R15.1–R15.5 COMPLETE + NORMALIZED; R15.6 IN_PROGRESS; R15.7–R15.17 PLANNED."
new = "R15.1–R15.5 COMPLETE + NORMALIZED; R15.6 COMPLETE; R15.7–R15.17 PLANNED."
if text.count(old) != 1:
    raise SystemExit(f"unexpected continuity header count: {text.count(old)}")
text = text.replace(old, new, 1)
lines = text.splitlines()
entry = (
    f"- R15.6 : **COMPLETE** — clean START / branch point `{START_SHA}`; continuity START-sync preceded implementation "
    "while the R15_PLAN checkpoint/table was inadvertently left stale and is repaired explicitly at END rather than laundered; "
    f"immutable technical source `{TECHNICAL}`; technical R15.6 #2 / `33295649414` SUCCESS Ubuntu + Windows, "
    "R0 #2124 / `33295649494` SUCCESS Ubuntu + Windows, Python #2099 / `33295649527` SUCCESS 5/5, "
    "UI #2064 / `33295649458` SUCCESS; technical scope accepted: immutable suite/task/scorer/config identities, R3 compatibility, "
    "domain/critical scoring, failure isolation, resource metrics, strict comparisons, Draft 2020-12 report schema, R15.4 protected-holdout "
    "binding and safe prompt/response digests; manual NONE. Fresh exact-head R15.6/R0/Python/UI on the repaired final END tree remain mandatory "
    "before expected-head PR #307 merge, followed by exactly one continuity-only normalization before R15.7."
)
for index, line in enumerate(lines):
    if line.startswith("- R15.6 :"):
        lines[index] = entry
