...
def build_core_evidence(
    ...
    *,
    ...
    blockers = [f"missing-authority:{path}" for path in missing_paths]
    ...
    return {
        ...
        "blockers": blockers,
        "critical_veto": any(item["status"] != "PASS" for item in controls),
...
def finalize_integrated_report(
    ...
    if not isinstance(subdivisions, list) or len(subdivisions) != 10:
        ...
        seen = {item.get("subdivision") for item in subdivisions if isinstance(item, dict)}
        ...
        for item in subdivisions:
            if not isinstance(item, dict) or item.get("status") != "PASS":
                name = item.get("subdivision", "unknown") if isinstance(item, dict) else "unknown"
    ...
    if not isinstance(controls, list) or len(controls) != len(ADVERSARIAL_CONTROLS):
        ...
        for control in controls:
            if not isinstance(control, dict) or control.get("status") != "PASS":
                name = control.get("control", "unknown") if isinstance(control, dict) else "unknown"
    ...
    return {
        ...
        "subdivision_accounting": subdivisions if isinstance(subdivisions, list) else [],
