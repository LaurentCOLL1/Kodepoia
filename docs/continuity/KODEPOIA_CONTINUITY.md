...
- Core phase DoD is expected to be provable with already supported hosted Android API 36 emulator/build/package/signing/compliance seams and hosted macOS Xcode/SwiftUI/XCTest simulator seams plus deterministic local DeviceLab/KodeRelease/diagnostics bindings. Physical-device capability, live Play/App Store publication, production signing/provisioning credentials and live TestFlight/App Store Connect state are explicitly not part of the core PASS claim unless implementation proves a frozen phase requirement cannot otherwise be established.
- If such a physical/live-account-only semantic is genuinely required, freeze the exact candidate, set truthful `MANUAL_REQUIRED`, provide one bounded collector with prerequisites/commands/evidence/recovery/privacy guidance, and stop before R14. Never request credentials or private signing material in chat.
