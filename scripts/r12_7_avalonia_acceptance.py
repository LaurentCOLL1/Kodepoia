...
from kodepoia.desktop.avalonia import (
    AvaloniaAcceptanceResult,
    AvaloniaAdapter,
    canonical_avalonia_matrix,
    write_avalonia_acceptance_report,
)
...
def main() -> int:
    ...
    parser.add_argument("--output", default="docs/roadmap/R12_7_AVALONIA_ACCEPTANCE.json")
    parser.add_argument("--work", default=".kodepoia/r12_7_avalonia")
    args = parser.parse_args()
    expected_sha = os.environ.get("KODEPOIA_SOURCE_SHA", "").strip()
    work = (ROOT / args.work).resolve()
    staging = work / "staging"
    staging.mkdir(parents=True, exist_ok=True)
    adapter = AvaloniaAdapter(ROOT, staging)
    result = adapter.run_acceptance(canonical_sample_app(), canonical_avalonia_matrix())
    ...
    assert isinstance(result, AvaloniaAcceptanceResult)
