...
def _parse_artifact(value: str) -> WorkflowArtifactBinding:
    parts = value.split("|")
    if len(parts) != 5:
        raise argparse.ArgumentTypeError("artifact must be KIND|RUN_NAME|ARTIFACT_ID|NAME|SHA256")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build exact-head R14.17 CI authority after independent gates pass.")
    ...
    parser.add_argument("--output", default="docs/roadmap/R14_17_CI_ACCEPTANCE.json")
    ...
    if tuple(item.kind for item in artifacts) != REQUIRED_ARTIFACT_KINDS:
        parser.error("--artifact values must appear once each in REQUIRED_ARTIFACT_KINDS order")
    ...
    print(json.dumps({"output": output.as_posix(), "evidence_sha256": evidence.evidence_sha256}, indent=2))
    return 0
