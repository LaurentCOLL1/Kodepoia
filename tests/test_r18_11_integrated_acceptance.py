...
def _populate_evidence(root: Path) -> Path:
    evidence = root / "evidence"
    evidence.mkdir(parents=True, exist_ok=True)
    for subdivision, filename in SUBDIVISION_EVIDENCE_FILES.items():
...
def _windows() -> dict[str, object]:
    return {
        ...
        "candidate_clean_install_uninstall_clean": True,
...
def test_core_evidence_accounts_for_all_subdivisions(tmp_path: Path) -> None:
    ...
    evidence = _populate_evidence(tmp_path)
    ...
    assert all(item["evidence_source_match"] for item in report["subdivision_accounting"])
...
def test_core_evidence_fails_closed_on_stale_subdivision_evidence(tmp_path: Path) -> None:
    _populate_authorities(tmp_path)
    evidence = _populate_evidence(tmp_path)
    stale = evidence / SUBDIVISION_EVIDENCE_FILES["R18.6"]
    ...
    assert "evidence-source-sha-mismatch:R18.6" in report["blockers"]
...
def test_integrated_report_vetoes_unexpected_negative_acceptance(tmp_path: Path) -> None:
    ...
    assert report["critical_veto"] is True
    assert any(
        "negative-control-unexpected-acceptance" in item
...
def test_integrated_report_vetoes_missing_clean_install(tmp_path: Path) -> None:
    _populate_authorities(tmp_path)
    evidence = _populate_evidence(tmp_path)
    core = build_core_evidence(
        tmp_path,
    ...
    windows = _windows()
    windows["candidate_clean_install_smoke"] = False
    report = finalize_integrated_report(SOURCE, core, windows)
    assert report["status"] == "FAIL"
    assert "windows-cycle-failed:candidate_clean_install_smoke" in report["blockers"]
